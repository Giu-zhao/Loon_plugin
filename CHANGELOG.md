...
- 按用户要求移除 YouTube Music 歌词翻译参数与网络调用；
- 插件不再向 Google Translate 发送歌词正文；
- 原生歌词内容保持不变，其余功能保持不变。

## 2.1.2 - 2026-08-24

- 按用户要求移除 App 与 Safari 字幕生成、翻译和强制选择功能；
- 完整保留 YouTube 原生字幕数据，字幕语言改为用户手动选择；
- 其余去广告、YouTube Music 加密流处理、后台播放和歌词翻译保持不变。

## 2.1.1 - 2026-08-24

- 修复 YouTube App 已选择简体中文但不展示字幕的问题；
- 同步设置列表级默认字幕轨道、音轨默认标记与开启状态；
- 翻译轨道不再错误继承原始自动字幕的 `asr` 类型。
- Safari 网页 player JSON 同步增加自动字幕翻译、默认中文轨道和字幕开启状态。
